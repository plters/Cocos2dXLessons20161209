...
void HelloWorld::addListeners(){
    ...
    auto submitBtnClickedListener = EventListenerTouchOneByOne::create();
    submitBtnClickedListener->onTouchBegan = handler;
    
    director->getEventDispatcher()->addEventListenerWithSceneGraphPriority(l, tf);
    director->getEventDispatcher()->addEventListenerWithSceneGraphPriority(submitBtnClickedListener, submitBtn);
}
